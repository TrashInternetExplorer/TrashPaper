...
function Library:object(class, properties)
    ...
    function methods:tween(options, callback)
        ...
        local _TweenInfo = TweenInfo.new(options.Length, options.Style, options.Direction)
        options.Length = nil
        options.Style = nil
        options.Direction = nil

        local Tween = TweenService:Create(localObject, _TweenInfo, options)
        Tween:Play()
    ...
    function methods:fade(state, colorOverride, length, instant)
        ...
        if not rawget(self, "fadeFrame") then
            ...
            frame:round(self.AbsoluteObject:FindFirstChildOfClass("UICorner") and self.AbsoluteObject:FindFirstChildOfClass("UICorner").CornerRadius.Offset or 0)
    ...
    for property, value in next, properties do
		if customProp[property] then
			customProp[property](value)
		else
			localObject[property] = value
		end
	end
...
function Library:create(options)
    ...
    if readfile and writefile and isfile then
        ...
        settings = HttpService:JSONDecode(readfile("IESettings.json"))
        ...
        options.Link = "https://paperhub.com/" .. options.Name .. "/"
    ...
    until not game.CoreGui.RobloxGui:FindFirstChild("Voice Chat Ui")
    ...
    core:tween({ Size = options.Size, Length = 0.5 }, function()
    ...
    do
		...
		if S then
            ...
            profilePictureCOntainer:round(100)
            ...
            do
                ...
                do
                    ...
                    local counter = 0
                    RunSerivce.Heartbeat:Connect(function(deltaTime)
                        counter += deltaTime
                ...
                settingsTabIcon:tooltip("Settings")
                ...
                settingsTab:button({
                    ...
                    Callback = function()
                        if writefile then
                            writefile("IESettings.json", HttpService:JSONEncode({
                                Theme = "Dark"
                            }))
                        end
...
function Library:tab(options)
    ...
    if not options.Internal then
        ...
        quickAccessButton:round(5):tooltip(options.Name)
